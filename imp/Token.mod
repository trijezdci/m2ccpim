...
TYPE
  ResWords = TokenT [Alias..Yield];
  ProcBindables = TokenT [Abs..Writef];
  ConstBindables = TokenT [Tbase..Trefc];
  Identifiers = TokenT [Abs..OtherIdent];
...
PROCEDURE isConstBindableIdent ( t : TokenT ) : BOOLEAN;
 (* Returns TRUE if t is a constant bindable identifier, otherwise FALSE. *)
BEGIN
  RETURN (t >= MIN(ConstBindables) AND t <= MAX(ConstBindables))
END isResWord;


PROCEDURE isProcBindableIdent ( t : TokenT ) : BOOLEAN;
 (* Returns TRUE if t is a procedure bindable identifier, otherwise FALSE. *)
BEGIN
  RETURN (t >= MIN(ProcBindables) AND t <= MAX(ProcBindables))
END isResWord;
